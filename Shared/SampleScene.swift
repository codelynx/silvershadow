...
class SampleScene: Scene {
	...
	lazy var sampleStroke: PointsRenderable = {
		...
		return PointsRenderable(device: self.device, texture: self.pointTexture, cgPath: cgPath, width: 32)!
	}()
...
//		let cgPath1 = CGMutablePath()
//		var lastPoint: CGPoint?
//		for point in points.map({ CGPoint(x: $0.0, y: $0.1) }) {
//			if let lastPoint = lastPoint {
//				let midPoint = (lastPoint + point) * 0.5
//				cgPath1.addQuadCurve(to: midPoint, control: lastPoint)
////				cgPath.addLine(to: CGPoint(point))
//			}
//			else {
//				cgPath1.move(to: point)
//			}
//			lastPoint = point
//		}
//		renderer.render(context: context, cgPath: cgPath1, texture: pointTexture, width: 32)

//		let cgPath2 = XBezierPath(ovalIn: CGRect(x: 50, y: 50, width: 1000, height: 320)).cgPath
//		renderer.render(context: context, cgPath: cgPath2, texture: pointTexture, width: 32)
